...
pub trait GameScContract: multiversx_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule {
    ...
    // #[view(getTravelerRewards)]
    // fn get_travelers_yield(&self, user: &ManagedAddress) -> BigUint {
    //     let mut travelers_rewards = BigUint::from(0 as usize);

    //     for nonce in self.staked_traveler_nonces(user).iter() {
    //         let nonce_reward = BigUint::from(block_diff * self.get_energy_yield(self.rarity_class(nonce).get()));
    //         travelers_rewards += nonce_reward;
    //     }
