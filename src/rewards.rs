...
pub trait Rewards: storage::Storage + helpers::Helpers {
    ...
    fn claim_reward(&self) {
        ...
        let mut available_nonces: UnorderedSetMapper<Self::Api, u16> =
            self.elders_tickets_nonces(self.current_trial().get());

        for nonce in self
            .get_staked_nonces(&caller, self.elders_mapper().get_token_id())
            .iter()
        {
    ...
    #[only_user_account]
    #[endpoint(claimEldersReward)]
    fn claim_elders_reward(&self) {
        let caller = self.blockchain().get_caller();
    ...
    fn get_elder_rewards(&self, user: &ManagedAddress) -> usize {
        let mut tickets_amount: usize = 0;
        let available_nonces: UnorderedSetMapper<Self::Api, u16> =
            self.elders_tickets_nonces(self.current_trial().get());
